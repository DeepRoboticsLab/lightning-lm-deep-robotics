...
### 第 2 步：解压 Pangolin-0.9.3 并且进行编译
进入 thirdparty 目录，解压 Pangolin：
```bash
...
sudo ldconfig
```
如果编译过程中没有报错，即表示 Pangolin-0.9.3 已完成编译和安装。
### 第 3 步：编译 lightning-lm
...
运行 `loc_online` 时，首先检查：`map_path`是否正确。
