...
tmux send-keys -t lg:0 "$ros2sh && ros2 run lightning run_slam_online --config src/lightning-lm/config/default_deep_roboticsslam.yaml" C-m
...
tmux send-keys -t lg:1 "$ros0sh && rviz2 -d src/lightning-lm/config/showbodypc.rviz" C-m
# tmux send-keys -t lg:1 "source install/setup.bash && rviz2 -d src/lightning-lm/config/showbodypc.rviz" C-m

# Tab 2: Check node info, save map service
tmux new-window -t lg:2 -n 'node_info'
tmux send-keys -t lg:2 "$ros0sh && ros2 topic echo /lightning/nav_state" C-m
